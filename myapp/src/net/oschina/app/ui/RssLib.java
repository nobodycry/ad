 ...
 * @created 2012-3-21
 */
public class RssLib extends BaseActivity{
    ...
    private void initView()
    {
    	...
    	software_recommend.setOnClickListener(this.softwareBtnClick(software_recommend,HEAD_TAG_RECOMMEND,"推荐NH"));
    	...
    	mSoftwareCatalogHandler = new Handler()
		{
			public void handleMessage(Message msg) {
				...
				if(msg.what >= 0){						
					...
					//发送通知广播
					if(notice != null){
						UIHelper.sendBroadCast(RssLib.this, notice);
					}
				}
				else if(msg.what == -1){
					//有异常--显示加载出错 & 弹出错误消息
    	...
    	mlvSoftwareTag.setOnItemClickListener(new AdapterView.OnItemClickListener() {
        	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {       		
        		...
        		if(type.tag > 0){
        			...
        			UIHelper.ToastMessage(RssLib.this, curSearchTag+type.encode);
    ...
    private void initSoftwareListView()
    {
    	...
    	mlvSoftware.setOnItemClickListener(new AdapterView.OnItemClickListener() {
        	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        		...
        		//RssLib.this.showNoticeDialog(RssLib.this,sw.name+"\n------\n"+sw.description+"\n------\n"+sw.url+"\n"+AppContext.lastFilePath);
        		
        		Intent intent = new Intent(RssLib.this, RssDetail.class);
			...
			public void handleMessage(Message msg) {
				...
				if(msg.what >= 0){						
					...
					case UIHelper.LISTVIEW_ACTION_CHANGE_CATALOG:
						...
						//UIHelper.ToastMessage(RssLib.this, "updated"+ApiClient.lastPageUrl);
						break;
					case UIHelper.LISTVIEW_ACTION_SCROLL:
						lvSumData += msg.what;
